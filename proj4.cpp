...
void testDisk()
{
  ...
  std::cout << disk.storeFile("def.txt", 4112, &str3[0]) << std::endl;
  disk.printCluster();
  std::cout << disk.readFile("def.txt", 16, 32) << std::endl;

  std::cout << "should say error => " << disk.storeFile("abc.txt", 0, NULL) << std::endl;
  std::cout << "should say error => " << disk.deleteFile("nothere.txt") << std::endl;

  std::cout << disk.dir() << std::endl;
